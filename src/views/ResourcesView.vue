...
import { policyDocs, fundingGuides, dockingMeetings } from '@/data/mock'
...
const filteredPolicies = computed(() => {
  if (!keyword.value) return policyDocs
  return policyDocs.filter(item => item.title.includes(keyword.value) || item.keywords.some(key => key.includes(keyword.value)))
})
</script>
